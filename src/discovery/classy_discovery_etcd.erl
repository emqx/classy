...
etcd_get(Servers, Key, Params, HttpOpts) ->
  classy_httpc:get(scheme(rand_addr(Servers)), Key, Params, HttpOpts).

etcd_set(Servers, Key, Params, HttpOpts) ->
  classy_httpc:put(scheme(rand_addr(Servers)), Key, Params, HttpOpts).

etcd_del(Servers, Key, Params, HttpOpts) ->
  classy_httpc:delete(scheme(rand_addr(Servers)), Key, Params, HttpOpts).

nodes_path(Options) ->
  with_prefix(Options, "/nodes").

node_path(Options) ->
  with_prefix(Options, "/nodes/" ++ atom_to_list(node())).

lock_path(Options) ->
  with_prefix(Options, "/lock").

with_prefix(Options, Path) ->
  Prefix = config(prefix, Options),
  Cluster = config(cluster_name, Options),
