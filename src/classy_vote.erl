...
-type votes() :: [{classy:site(), boolean()}].
-define(s_vote, 0).
-define(s_commit, 10).
...
-spec participant_pre_vote(prepare()) -> {ok, boolean()}
                                       | {error, _}.
...
init_coordinator(#init_coordinator{tag = Tag, id = Id, opts = Opts}) ->
  D = #d_coord{tag = Tag, id = Id, votes = [], opts = Opts},
...
coordinator_enter(
  ...
  #d_coord{id = Id, opts = Opts}
 ) ->
  ok = db_set_coord_state(Id, State),
  case Stage of
    ?s_vote ->
      coordinator_perform_vote(Id, Opts);
    ?s_commit ->
...
-spec coordinator_perform_vote(id(), options()) ->
        {keep_state_and_data, [gen_statem:action()]}.
coordinator_perform_vote(Id, Options) ->
  #{ tag      := Tag
   , lock     := Lock
   , prepare  := Prepare
   , commit   := Commit
  ...
  {keep_state_and_data, [{state_timeout, ?state_timeout, Timeout}]}.
