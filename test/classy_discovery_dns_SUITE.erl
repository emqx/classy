...
-module(classy_discovery_dns_SUITE).
...
init_per_testcase(t_discover, Config) ->
  ...
  meck:expect(
    ...
    fun(_, _, a) ->
        [{127, 0, 0, 1}];
       (_, _, srv) ->
        [ {0, 6, 8083,  "emqx-0.emqx.default.svc.cluster.local"}
       ...
       (Name, Class, Type) ->
        meck:passthrough([Name, Class, Type])
      end),
  Config;
...
t_discover(_) ->
    Options1 = #{name => "localhost", app => "classy"},
    {ok, ['classy@127.0.0.1']} = classy_discovery_strategy:discover(classy_discovery_dns, Options1),

    Options2 = #{name => "emqx.default.svc.cluster.local", app => "classy", type => srv},
    {ok, [ 'classy@emqx-0.emqx.default.svc.cluster.local'
         , 'classy@emqx-1.emqx.default.svc.cluster.local'
         , 'classy@emqx-2.emqx.default.svc.cluster.local'
